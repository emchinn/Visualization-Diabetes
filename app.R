...
Diabetes <- read.csv("data/diabetic_data.csv")
...
my_checkboxGroupInput <- function(variable, label, choices, selected, colors){
  ...
  my_names <- names(choices) # if(length(names(choices))>0) 
...
my_selected <- c("Pediatrics", "Obstetrics/Gynecology") # initialize checkbox selection to just 2
...
server <- function(input, output) {
  ...
  Data <- reactive({
    # filter to the desired specialty
    if(length(input$med_spec) == 0){
      return(NULL)
    }else {
